...
from content_navigation import service_expansion
...
services = [
  ...
  {"slug":"ppf","title":"PPF para Toyota en Cartagena","desc":"Instalación de película protectora PPF para pintura Toyota en zonas de impacto o cobertura completa.","image":"service-ppf-listing.webp","intro":"La película PPF crea una barrera física transparente frente a gravilla, insectos y roces superficiales. Evaluamos pintura, cobertura y terminaciones antes de instalar.","includes":["Inspección, lavado y descontaminación de pintura","Corrección previa acordada cuando sea necesaria","Cobertura frontal, zonas de contacto o proyecto completo","Terminación de bordes e instrucciones de curado y lavado"],"process":["Definimos cobertura y comprobamos estabilidad de la pintura.","Preparamos la superficie y aplicamos patrones con tensión controlada.","Revisamos bordes, contaminación y acabado después del asentamiento inicial."],"faqs":[["¿Cuánto cuesta instalar PPF en Cartagena?","Depende del modelo, cobertura, película, geometría y condición de la pintura. Inspeccionamos el vehículo y entregamos una cotización con paneles, preparación, desmontajes e impuestos claramente separados."],["¿Puedo proteger solo el frente?","Sí. Se puede cotizar frontal, zonas de contacto o cobertura completa según uso y presupuesto."],["¿Cuánto tarda la instalación?","Depende del alcance. Como referencia, una cobertura completa de Land Cruiser 250 requiere aproximadamente una semana entre preparación, instalación, armado y revisión."],["¿Hay que desarmar el vehículo?","Solo se retiran de forma controlada las piezas necesarias para acceder a ciertos bordes. El desmontaje se define y documenta después de revisar el modelo y sus accesorios."],["¿El PPF evita cualquier daño?","No. Ayuda frente a desgaste e impactos ligeros, pero no evita abolladuras, accidentes o cortes profundos."],["¿Se instala sobre pintura reparada?","Solo después de evaluar la estabilidad y el curado. Un repinte deficiente puede fallar durante la instalación o el retiro."],["¿Puede combinarse con cerámico?","Sí, si ambos sistemas son compatibles y se define el orden correcto."],["¿Qué debo recibir con la garantía?","La marca y línea de película, paneles cubiertos, plazo, responsable, mantenimiento y exclusiones deben quedar por escrito."]]},
...
commercial = {
  ...
  "repuestos": {
    "meta_title": "Repuestos Toyota en Cartagena | Cotiza por referencia",
    "meta_desc": "Cotiza repuestos Toyota en Cartagena: filtros, frenos, suspensión y motor. Identificación por VIN, marca y disponibilidad confirmadas por WhatsApp.",
    ...
    "local_paragraphs": [
      "Gestionamos solicitudes para Corolla, Yaris, RAV4, Prado, Fortuner, Hilux y otros modelos atendidos por el taller. La disponibilidad cambia por referencia. Confirmamos marca, plazo informado por el proveedor y alcance de instalación antes del pago o autorización.",
...
# Commercial questions must stay identical in the visible FAQ and JSON-LD.
for service in services:
    if service['slug'] == 'repuestos':
        service['faqs'] += [
            ['¿Tienen repuestos para Prado, Fortuner e Hilux?', 'Puedes consultar repuestos para estos modelos por año, motor, VIN y referencia. Confirmamos compatibilidad, marca y disponibilidad de cada pieza antes de cotizar.'],
            ['¿Cuánto cuesta un repuesto Toyota y cuánto tarda?', 'Depende de la referencia, marca, proveedor y disponibilidad. Comparte los datos del vehículo y la pieza para recibir precio y plazo; no todos los repuestos están disponibles para entrega inmediata.'],
        ]
    if service['slug'] == 'mantenimiento-general':
        service['faqs'] += [
            ['¿Qué incluye el mantenimiento de 40.000 u 80.000 km?', 'El contenido depende del manual de la versión y de los servicios ya realizados. Revisamos tiempo, kilometraje, historial y condición antes de definir aceite, filtros, inspecciones o cambios adicionales.'],
            ['¿Cuánto cuesta el mantenimiento de mi Toyota?', 'El precio se define por modelo, motor y alcance. Solicita una cotización que separe mano de obra, fluidos, filtros y repuestos; una reparación detectada en la revisión requiere un alcance adicional autorizado.'],
        ]
commercial['diagnostico-electronico']['related'].append(('Causas de pérdida de potencia Toyota', '/blog/toyota-pierde-potencia-causas-diagnostico/'))
commercial['motor-refrigeracion']['related'].append(('Toyota se recalienta al usar el aire acondicionado', '/blog/toyota-se-recalienta-con-aire-acondicionado/'))
commercial['frenos-suspension']['related'].append(('Toyota vibra al frenar: causas y revisión', '/blog/toyota-vibra-al-frenar-causas/'))
commercial['transmision']['related'].append(('Revisión y cambio del aceite de transmisión', '/blog/cambio-aceite-transmision-automatica-toyota/'))
...
for service in services:
    ...
    whatsapp_message = quote(
        f"Hola Toyo Services. Quiero cotizar {service_label.lower()} para mi Toyota. Modelo: __. Año: __. Kilometraje: __. Detalle o referencia: __."
    )
    ...
    image_attrs = responsive_attrs(service["image"], "(max-width: 700px) calc(100vw - 40px), 1180px")
    page=f'''<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(content['meta_title'])}</title><meta name="description" content="{html.escape(content['meta_desc'])}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="{canonical}"><meta property="og:type" content="website"><meta property="og:locale" content="es_CO"><meta property="og:site_name" content="Toyo Services"><meta property="og:title" content="{html.escape(service['title'])}"><meta property="og:description" content="{html.escape(content['meta_desc'])}"><meta property="og:image" content="{BASE}/assets/images/{service['image']}"><meta property="og:url" content="{canonical}"><meta name="twitter:card" content="summary_large_image"><link rel="icon" href="/favicon.ico" type="image/x-icon" sizes="48x48"><link rel="apple-touch-icon" href="/assets/images/apple-touch-icon.png" sizes="180x180"><link rel="manifest" href="/site.webmanifest"><link rel="stylesheet" href="/assets/css/styles.min.css?v=24"><script type="application/ld+json">{json.dumps(schema,ensure_ascii=False)}</script><script type="application/ld+json">{json.dumps(faq,ensure_ascii=False)}</script><script type="application/ld+json">{json.dumps(breadcrumb,ensure_ascii=False)}</script></head><body>{header}<main><section class="service-detail-hero"><div class="container"><nav class="breadcrumb" aria-label="Migas de pan"><a href="/">Inicio</a> / <a href="/servicios/">Servicios</a> / {html.escape(service['title'])}</nav><span class="eyebrow">Servicio especializado</span><h1>{html.escape(service['title'])}</h1><p>{html.escape(service['intro'])}</p><div class="actions"><a class="btn btn-primary" href="https://wa.me/573018638164?text={whatsapp_message}" target="_blank" rel="noopener">Cotizar por WhatsApp</a><a class="btn btn-outline" href="/#ubicacion">Ver área de atención</a></div></div></section><section class="service-detail-body"><div class="container"><img class="service-detail-image" loading="lazy" src="/assets/images/{service['image']}" {image_attrs} width="1536" height="1024" alt="{html.escape(service['title'])}"><div class="service-detail-grid"><div><span class="eyebrow">Cuándo revisar</span><h2>{html.escape(content['signs_heading'])}</h2>{sign_paragraphs}<ul class="detail-checklist">{signals}</ul></div><div><span class="eyebrow">Qué incluye</span><h2>{html.escape(content['scope_heading'])}</h2><ul class="detail-checklist">{includes}</ul></div><div><span class="eyebrow">Proceso de trabajo</span><h2>{html.escape(content['process_heading'])}</h2><ol class="detail-process">{process}</ol></div></div></div></section><section class="service-detail-body"><div class="container"><div class="service-detail-grid"><div><span class="eyebrow">Contexto local</span><h2>{html.escape(content['local_heading'])}</h2>{local_paragraphs}</div><div><span class="eyebrow">Alcance responsable</span><h2>{html.escape(content['parts_heading'])}</h2>{parts_paragraphs}</div><div><span class="eyebrow">Antes de venir</span><h2>{html.escape(content['bring_heading'])}</h2><p>{html.escape(content['bring_intro'])}</p><ul class="detail-checklist">{bring}</ul></div></div><div class="service-detail-faq"><span class="eyebrow">Información relacionada</span><h2>Guías relacionadas con {html.escape(service_label)}</h2><ul>{related}</ul></div></div></section><section class="faq"><div class="container service-detail-faq"><span class="eyebrow">Preguntas del servicio</span><h2>Preguntas frecuentes sobre {html.escape(service_label)}</h2>{faqs}</div></section><section class="service-quote"><div class="container"><div><span class="eyebrow">Evaluación previa</span><h2>Siguiente paso para {html.escape(service_label)}</h2><p>El alcance final depende del modelo, año, versión, historial y condición comprobada del vehículo.</p></div><div class="actions"><a class="btn btn-primary" href="https://wa.me/573018638164?text={whatsapp_message}" target="_blank" rel="noopener">Cotizar por WhatsApp</a><a class="btn btn-outline" href="/#ubicacion">Ver área de atención</a></div></div></section></main>{footer}</body></html>'''
    expansion = service_expansion(service['slug'])
    if expansion:
        page = page.replace('<section class="faq">', expansion + '<section class="faq">', 1)
        anchor, label = ('categorias-repuestos', 'Ver repuestos por sistema') if service['slug'] == 'repuestos' else ('plan-mantenimiento', 'Ver el plan de mantenimiento')
